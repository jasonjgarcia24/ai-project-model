#!/usr/bin/env python3
"""Build insertTableRow + insertText batchUpdate requests from document structure.
...
Arguments:
    ...
    table_data_file  Path to /tmp/kickoff_table_data.json (from populate_kickoff.py)

Output files:
    /tmp/kickoff_insert_rows.json   — batchUpdate body for insertTableRow requests
    /tmp/kickoff_insert_cells.json  — batchUpdate body for insertText into cells
                                      (must be run AFTER rows are inserted + fresh GET)

Usage workflow:
    1. GET the document → save to /tmp/kickoff_doc.json
    2. Run: python build_table_inserts.py /tmp/kickoff_doc.json /tmp/kickoff_table_data.json
    3. POST /tmp/kickoff_insert_rows.json as batchUpdate body
    4. GET the document again → save to /tmp/kickoff_doc.json (fresh indices)
    5. Run: python build_table_inserts.py /tmp/kickoff_doc.json /tmp/kickoff_table_data.json
    6. POST /tmp/kickoff_insert_cells.json as batchUpdate body
"""
...
# ---------------------------------------------------------------------------
# Metrics tables all share the same header ("Metric ID"), so we match by order:
# 1st "Metric ID" table = technical, 2nd = human-centered, 3rd = business.
# Risks table has a unique header ("Risk ID").

METRICS_KEYS_IN_ORDER = [
    {"key": "technical_metrics", "col_keys": ["id", "metric", "threshold", "action_if_breached"]},
    {"key": "human_centered_metrics", "col_keys": ["id", "metric", "threshold", "action_if_breached"]},
    {"key": "business_metrics", "col_keys": ["id", "metric", "threshold", "action_if_breached"]},
]
RISKS_SPEC = {"key": "risks", "col_keys": ["risk_id", "description", "likelihood", "impact", "mitigation", "owner"]}
...
def classify_tables(tables: list[dict]) -> list[tuple[dict, dict]]:
    """Match tables to their specs using order-based logic.

    Returns list of (table, spec) tuples where spec has 'key' and 'col_keys'.
    Metrics tables (header contains 'Metric ID') are assigned in document order:
    1st = technical, 2nd = human-centered, 3rd = business.
    Risks table (header contains 'Risk ID') is matched uniquely.
    """
    matched = []
    metrics_idx = 0
    ...
    for t in tables:
        ...
        if "Risk ID" in header:
            matched.append((t, RISKS_SPEC))
        elif "Metric ID" in header and metrics_idx < len(METRICS_KEYS_IN_ORDER):
            matched.append((t, METRICS_KEYS_IN_ORDER[metrics_idx]))
            metrics_idx += 1
...
def get_last_row_index(table: dict) -> int:
    ...
    if not rows:
        return -1
    # Use the last row's first cell's startIndex
    last_row = rows[-1]
...
def build_row_insert_requests(tables: list[dict], table_data: dict) -> dict:
    ...
    for table, spec in classified:
        ...
        existing_rows = len(table.get("table", {}).get("tableRows", [])) - 1  # minus header
...
def main():
    ...
    Path("/tmp/kickoff_insert_rows.json").write_text(json.dumps(row_requests))
    ...
    Path("/tmp/kickoff_insert_cells.json").write_text(json.dumps(cell_requests))
    ...
    summary = {
        ...
        "files": {
            "insert_rows": "/tmp/kickoff_insert_rows.json",
            "insert_cells": "/tmp/kickoff_insert_cells.json",
        },
    }
